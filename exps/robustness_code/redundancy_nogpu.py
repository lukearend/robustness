...
for cross in range(3):
    ...
    with open(opt.log_dir_base + opt.name + '/kernel' + str(cross) +'.pkl', 'wb') as f:
        pickle.dump(kernel, f, protocol=2)

    with open(opt.log_dir_base + opt.name + '/kernel_test' + str(cross) +'.pkl', 'wb') as f:
        pickle.dump(kernel_test, f, protocol=2)

    with open(opt.log_dir_base + opt.name + '/NN' + str(cross) +'.pkl', 'wb') as f:
