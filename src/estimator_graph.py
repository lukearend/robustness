...
def forward_pass(x, is_training, params):
    x = tf.layers.conv2d(x, filters=20, kernel_size=5, stride=1)
    x = tf.layers.relu(x)
    x = tf.layers.max_pool2d(x, pool_size=2, stride=2)
    x = tf.layers.conv2d(x, filters=50, kernel_size=5, stride=1)
    x = tf.layers.relu(x)
    x = tf.layers.max_pool2d(x, pool_size=2, stride=2)
